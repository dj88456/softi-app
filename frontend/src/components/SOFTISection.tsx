...
function RichTextInput({
  ...
  return (
    <div className={`rounded border bg-white focus-within:ring-1 focus-within:ring-indigo-400 focus-within:border-indigo-400 border-gray-200 transition ${className ?? ''}`}>
      {/* Toolbar — visible when focused or non-empty */}
      {showToolbar && (focused || value) && (
        <div className="flex flex-wrap gap-0.5 px-1.5 pt-1.5 pb-1 border-b border-gray-100">
          {TOOLBAR.map(btn => (
            <button
              key={btn.label}
              type="button"
